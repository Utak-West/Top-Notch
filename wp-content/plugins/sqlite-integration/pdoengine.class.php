...
class PDOEngine extends PDO {
	 ...
	 * @see PDO::query()
	 */
	// Fixed method signature for PHP 8 compatibility
	public function query(string $query, ?int $fetchMode = null, mixed ...$fetchModeArgs): \PDOStatement|false {
      ...
      // Ensure false is returned on failure, not null, to match PDOStatement|false return type
	...
	private function replace_variables_with_placeholders($matches) {
		...
		// Fixed curly brace syntax for PHP 8 compatibility
		// Using square brackets instead
		$last_char_index = strlen($param) - 1;
		$first_char_index = 0;
		
		// Check if the string is not empty before accessing characters
		$last_char = ($last_char_index >= 0) ? $param[$last_char_index] : null;
		$first_char = ($last_char_index >= 0) ? $param[$first_char_index] : null;

		// Remove quotes if they exist using trim
		$param = trim($param, "'\"");

		//$this->extracted_variables[] = $param;
		$key = ':param_'. $this->param_num++;		$this->extracted_variables[] = $param;
